...
interface AllKaryaPopupProps {
  isOpen: boolean;
  onClose: () => void;
  works: WorkItem[];
}

export default function AllKaryaPopup({ isOpen, onClose, works }: AllKaryaPopupProps) {
  useEffect(() => {
    if (isOpen) {
            ...
            <div className="relative z-10 p-8 md:p-[78px]">
              <div className="flex flex-col">
                ...
                <p className="text-[#424345] text-base md:text-[18px] leading-relaxed md:leading-[29px] mb-12">
                  Berikut adalah kumpulan mahakarya terbaik dari talenta kreatif MUDENG. Setiap karya mencerminkan dedikasi, inovasi, dan perpaduan seni serta teknologi digital.
                </p>
                ...
                <div className="flex flex-col gap-16 md:gap-20">
                  {works.map((work, i) => (
                    <div key={i} className="flex flex-col gap-6">
                      ...
                      <img src={work.image} alt={work.title} className="w-full h-auto object-cover rounded-[20px] md:rounded-[30px]" />
                    </div>
                  ))}
                </div>
              </div>
